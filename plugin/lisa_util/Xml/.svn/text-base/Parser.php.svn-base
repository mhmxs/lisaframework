...
class Parser
{
    ...
    */
    ...
    public static function initByFilePath($filename, $cleanTagNames = true)
    {
    	...
    	return new $class($filename, $cleanTagNames);
    }
    ...
    {
    	$class = get_called_class();
		return new $class($content, $cleanTagNames, false);
    }
    ...
    public function __construct($XMLFile, $cleanTagNames = true, $readContent = true)
    {
    	if ($readContent == true) {
	        //Load XML document
	        $file = new \Util\FileHandler( $XMLFile );
    ...
    {
		$msg = 'XML Parsing Error at '.$line.':'.$col.'. Error '.$code.': '.xml_error_string($code);

		$eh = \Context::getService('ErrorHandler');
		if (!is_null($eh)) {
			$eh->trace($msg, \E_USER_WARNING);
		} else {
			trigger_error($msg);
		}
    }
    ...
    protected function _StartElement($parser, $name, $attrs = array())
    {
        ...
        if (count($this->_stack) == 0)
        {
            ...
            $this->_document = new Tag($name, $attrs);
