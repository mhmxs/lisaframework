...
class ImageModifier {
	...
	protected function _resizeFill() {
		$x = $this->_width < $this->_newWidth ? $this->_width : $this->_width  / ($this->_height / $this->_newHeight);
		$y = $this->_height < $this->_newHeight ? $this->_height : $this->_height / ($this->_width / $x);

		if ($y > $this->_newHeight) {
			$y = $height;
			$x = $this->_width  / ($this->_height / $y);
		}

		$imgtemp = $this->_protectAlphaColors($x, $y);
		imagecopyresampled( $imgtemp, $this->_img, 0, 0, 0, 0, $x, $y, $this->_width, $this->_height );
		$this->_img	= $imgtemp;
	}
	...
	public function watermarkFixPosition( $image, $position, $alfa = 100,$percent = 30 ) {
		...
		switch( $position ) {
			...
			case "middle-center":
				$x = ( $this->_width - $this->_wmWidth ) / 2;
				$y = ( $this->_height / 2 ) - ( $watermark_h / 2 );
				break;
			case "middle-right":
				$x = $this->_width - $this->_wmWidth;
				$y = ( $this->_height / 2) - ( $this->_wmHeight / 2 );
