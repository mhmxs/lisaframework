...
class ARO
{
	...
	public function __construct($unique_id = null, $group = false)
	{
		...
		$adapter        = "ARO" . ucwords(strtolower($this->_storeEngine));
	...
	{
		$reader = new ConfigReader(DIR_CONFIG . "/Config.ini");

		if (!is_null($reader->ACL->store_engine)) {
			$this->_storeEngine = $reader->ACL->store_engine;
		} else {
			throw new ACLException("No configuration for ACL in Config.ini!");
		}
	}
}
